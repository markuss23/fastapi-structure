...
async def get_book(id: int, db: Session = Depends(get_db)):
    db_book = (
        db.query(Book)
        .options(joinedload(Book.authors).options(joinedload(BookAuthor.author)))
        .where(Book.id == id)
        .one()
    )
    return db_book
...
async def get_author(id: int, db: Session = Depends(get_db)):
    db_author = (
        db.query(Author)
        .options(joinedload(Author.books).options(joinedload(BookAuthor.book)))
        .where(Author.id == id)
        .one()
    )
    return db_author
...
async def get_authoras(db: Session = Depends(get_db)):
    db_authors = (
        db.query(Author)
        .options(joinedload(Author.books).options(joinedload(BookAuthor.book)))
        .all()
